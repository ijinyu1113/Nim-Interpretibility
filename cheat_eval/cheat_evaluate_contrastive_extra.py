"""Cheat eval, but ONLY for the 2 extra contrastive-only seeds {2, 7}.

Does NOT overwrite eval_results_summary.json. Outputs a separate file
eval_results_contrastive_extra.json in the same nested format, which can be
merged into the main summary with merge_cheat_eval_results.py.

Usage (from access_files/ on HPC):
    python -u cheat_eval/cheat_evaluate_contrastive_extra.py
"""
import torch
import json
import os
...
METHODS = {
    "Contrastive-only (lambda=1, no-paired)": {
        s: f"ijinyu1113/contrastive_l1.0_layer12_s150000_seed{s}_v3_nopaired"
...
def evaluate_model_detailed(model, tokenizer, samples, regime_name, cheat_pairs=None):
    correct = cheat_predicted = valid_move = total = 0
...
def main():
    ...
    for (m_name, seed), m_path in MODELS.items():
        ...
        for regime_name, samples in eval_sets.items():
            ...
            r = evaluate_model_detailed(model, tokenizer, samples, regime_name, use_cheat)
            final_results[m_name][seed][regime_name] = r
            print(f"  {regime_name}: move_acc={r['move_acc']}%  valid={r['valid_move_rate']}%"
                  + (f"  cheat={r['cheat_move_rate']}%" if "cheat_move_rate" in r else ""))

        del model
        torch.cuda.empty_cache()

    out_path = "eval_results_contrastive_extra.json"
    with open(out_path, "w") as f:
        json.dump(final_results, f, indent=2)
    print(f"\nResults saved to {out_path}.")
    print("Next: run merge_cheat_eval_results.py to fold into eval_results_summary.json")
