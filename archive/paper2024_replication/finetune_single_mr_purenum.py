...
LR_STR = sys.argv[4] if len(sys.argv) > 4 else "5e-6"
LR = float(LR_STR)
WD_STR = sys.argv[5] if len(sys.argv) > 5 else "0.1"
WD = float(WD_STR)
...
MAX_LENGTH = 128
MAX_STEPS = int(sys.argv[6]) if len(sys.argv) > 6 else 50000
EVAL_EVERY = 250
SAVE_EVERY = 10000
LOG_EVERY = 250
BATCH_SIZE = 64
...
HF_REPO = f"ijinyu1113/ft_mr{MAX_REMOVE}_{MODEL_SIZE}_seed{SEED}_lr{LR_STR}_wd{WD_STR}_purenum"
OUTPUT_DIR = f"/projects/benv/iyu1/ft_mr{MAX_REMOVE}_{MODEL_SIZE}_seed{SEED}_lr{LR_STR}_wd{WD_STR}_purenum"

METRICS_DIR = "new_result/purenum_metrics"
os.makedirs(METRICS_DIR, exist_ok=True)
METRICS_JSONL = f"{METRICS_DIR}/mr{MAX_REMOVE}_{MODEL_SIZE}_seed{SEED}_lr{LR_STR}_wd{WD_STR}.jsonl"

print(f"Config: max_remove={MAX_REMOVE}, seed={SEED}, model={MODEL_SIZE} ({REPO_ID}), lr={LR}, wd={WD}")
...
def compute_metrics(eval_pred):
    ...
    # CausalLM: pred_ids[t] = predicted token for position t+1 given input[0:t+1].
    # Shift so shifted_pred[t] is the model's prediction FOR position t.
