...
def plot_transition_perseed_2panel(outname):
    """2-panel per-seed eval figure: (a) 357->468, (b) 468->357.
    For each mr, draws individual seed curves as faint thin lines and the
    across-seeds median as a bold line."""
    configs = [
        ("357 $\\rightarrow$ 468_later", "357_468l", "357", "468_later"),
        ("468 $\\rightarrow$ 357_later", "468_357l", "468", "357_later"),
    ]
    ...
    for idx, (label, direction, first, second) in enumerate(configs):
        ax = axes[idx]
        files = find_seed_files(f"trans_{direction}")
        _report_seeds(f"trans_{direction} (perseed-2panel)", files)
        ...
        for ci, k in enumerate(buckets):
            color = BUCKET_COLORS[ci]
            # Skip buckets that never get above 1% across any seed
            if all(max(d["eval"][k]) < 1.0 for _, d in parsed_by_seed):
                continue

            # Faint per-seed lines
            for _, d in parsed_by_seed:
                if not d["steps_eval"]:
                    continue
                x = np.array(d["steps_eval"]) / 1000.0
                ax.plot(x, d["eval"][k], color=color, linewidth=0.7,
                        alpha=0.35, zorder=1)

            # Bold across-seeds median
            per_seed = [{"s": d["steps_eval"], "v": d["eval"][k]}
                        for _, d in parsed_by_seed]
            steps, med, _q1, _q3 = aggregate(per_seed, "s", "v")
            if steps is not None:
                ax.plot(steps / 1000.0, med, color=color, linewidth=2.2,
                        alpha=0.95, label=f"mr={k}", zorder=3)

            # Chance line per mr
            ax.axhline(100 / (k + 1), color=color,
                       lw=HLINE_KW["lw"], ls=HLINE_KW["ls"],
                       alpha=0.35, zorder=0)
    ...
    # Single shared legend above figure
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=len(buckets),
               bbox_to_anchor=(0.5, 1.05), frameon=False)
    fig.tight_layout(rect=[0, 0, 1, 0.92])
...
def plot_transition_train_perseed(direction, first, second, title, outname):
    """2-panel TRAIN-accuracy figure with median bold + faint per-seed lines.
    Panel (a): training accuracy on the Phase I dataset (`first`).
    Panel (b): training accuracy on the Phase II dataset (`second`).
    Mirrors transition_*_medianiqr.png layout but drops the eval panel and
    overlays per-seed faint lines under the median."""
    files = find_seed_files(f"trans_{direction}")
    _report_seeds(f"trans_{direction} (train-perseed)", files)
    parsed_by_seed = [(s, parse_transition_log(p, first, second))
                      for s, p in files.items() if p]
    if not parsed_by_seed:
        print(f"  skipping {outname}: no files")
    ...
    fig, axes = plt.subplots(1, 2, figsize=(7.2, 3.0), sharey=True)
    panels = [
        (f"Phase I: train on {first}",  "steps_tf", "train_first"),
        (f"Phase II: train on {second}", "steps_ts", "train_second"),
    ]
    panel_letters = ["(a)", "(b)"]
    ...
    for idx, (ax, (panel_title, skey, bkey)) in enumerate(zip(axes, panels)):
        for ci, k in enumerate(buckets):
            ...
            # Per-seed series for this panel
            seed_series = []
            for _, d in parsed_by_seed:
                steps = d[skey]
                vals = d[bkey][k]
                if not steps:
                    continue
                seed_series.append({"s": list(steps), "v": list(vals)})

            if not seed_series:
                continue
            if all(max(ss["v"]) < 1.0 for ss in seed_series):
                continue

            # Faint per-seed lines
            for ss in seed_series:
                ax.plot(np.array(ss["s"]) / 1000.0, ss["v"],
                        color=color, linewidth=0.7, alpha=0.35, zorder=1)

            # Bold across-seeds median
            steps, med, _q1, _q3 = aggregate(seed_series, "s", "v")
            if steps is not None:
                ax.plot(steps / 1000.0, med, color=color, linewidth=2.2,
                        alpha=0.95, label=f"mr={k}", zorder=3)
    ...
    # Shared legend covering all buckets seen across both panels (sorted by mr)
    seen = {}
    for ax in axes:
        for h, lbl in zip(*ax.get_legend_handles_labels()):
            if lbl not in seen:
                seen[lbl] = h
    sorted_labels = sorted(seen.keys(), key=lambda s: int(s.split("=")[1]))
    handles = [seen[l] for l in sorted_labels]
    fig.legend(handles, sorted_labels, loc="upper center", ncol=len(buckets),
               bbox_to_anchor=(0.5, 1.05), frameon=False)
    fig.tight_layout(rect=[0, 0, 1, 0.92])
    out = os.path.join(OUTPUT_DIR, outname)
    plt.savefig(out, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    print("\n=== Transition train per-seed (median + faint) ===")
    plot_transition_train_perseed("357_468l", "357", "468_later",
                                  "357 -> 468_later train",
                                  "transition_357_468later_train.png")
    plot_transition_train_perseed("468_357l", "468", "357_later",
                                  "468 -> 357_later train",
                                  "transition_468_357later_train.png")
    print("\n=== 3-method comparison ===")
