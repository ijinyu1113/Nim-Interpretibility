...
# CLI: optional first positional arg = model size tag (e.g. "410m" or "70m").
import sys
MODEL_SIZE = sys.argv[1] if len(sys.argv) > 1 else "410m"
SIZE_TAG_FOR_CSV = MODEL_SIZE.upper()  # "410M" / "70M"

METRICS_DIR = "new_result/purenum_metrics"
OUT_PLOT_DIR = "new_result/plots"
LONG_CSV_PATH = os.path.join(METRICS_DIR, f"purenum_combined_{MODEL_SIZE}.csv")
CSV_PATH = os.path.join(METRICS_DIR, f"icml2026 - {SIZE_TAG_FOR_CSV}.csv")
...
def gather_long():
    ...
    for path in sorted(glob.glob(os.path.join(METRICS_DIR, f"mr*_{MODEL_SIZE}_seed*.jsonl"))):
...
def aggregate_per_step(df, metric, step_multiple=500):
    ...
    with columns mr, step, n, median, q1, q3, lo, hi over the subset where
    metric is not NaN. lo/hi are min/max across seeds (used for error bands)."""
...
def plot_metric(agg, metric, title_metric, outpath):
    ...
    for ci, mr in enumerate(MRS):
        ...
        lo = sub["lo"].to_numpy()
        hi = sub["hi"].to_numpy()
        max_step_k = max(max_step_k, x.max())
    ...
    # Linear up to step 10k, log beyond. X-axis spans the actual data range.
    ax.set_xscale("symlog", linthresh=10, linscale=1)
    upper = int(np.ceil(max_step_k / 5.0) * 5) or 35  # round up to nearest 5
    ax.set_xlim(0, upper)
    # Ticks: linear region 0,5,10 + log-spaced beyond
    log_ticks = [t for t in (15, 20, 30, 50, 70, 100) if t <= upper]
...
def main():
    ...
    plot_metric(eval_agg,  "eval_acc",  "Eval move",  os.path.join(OUT_PLOT_DIR, f"purenum_eval_curves_{MODEL_SIZE}.png"))
    plot_metric(train_agg, "train_acc", "Train move", os.path.join(OUT_PLOT_DIR, f"purenum_train_curves_{MODEL_SIZE}.png"))
