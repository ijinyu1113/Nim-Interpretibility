"""Build the 3 x N logit-lens evolution figure across training checkpoints.

Rows: MLP / Attention / Residual stream (h^(l) = embed + sum Attn^i + sum MLP^i).
Columns: training checkpoints (subset of saved steps).
Shared x-axis (Layer), shared y-axis (Move). One colorbar per row, scaled to
the global min/max within that row so colors are comparable across columns.
"""
import glob
import os
import re
...
OUT = "new_result/plots/logit_lens_evolution.png"
MAX_REMOVE = 5
MOVE_LABELS = ["-1"] + [str(k) for k in range(1, MAX_REMOVE + 1)]
CORRECT_MOVE = "4"  # for example_prompt.txt (n=52, mod 6 = 4)
# Subset of checkpoints to display. None = all available.
STEPS_SUBSET = [1000, 3000, 5000, 7000, 9000, 11000]
...
def main():
    ...
    # Per-row colorbars on the LEFT, labeled with the row's logit type
