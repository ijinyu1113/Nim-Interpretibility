...
def plot_set(config):
    ...
    ncols = -(-n // nrows)  # ceil division
    # Each cell is a unit square; content aspect is n_layers : n_strat.
    cell_in = 0.22  # inches per cell
    content_h_per_panel = cell_in * n_strat
    content_w_per_panel = cell_in * n_layers
    left_pad = 1.5   # strategy labels on leftmost panel
    right_pad = 0.7  # thin colorbar
    row_pad = 0.6    # vertical gap per row for x-labels
    fig_w = content_w_per_panel * ncols + left_pad + right_pad
    fig_h = content_h_per_panel * nrows + row_pad * nrows + 0.6
    ...
    for idx, (label, mat) in enumerate(mats):
        ...
        if idx >= (nrows - 1) * ncols:
            ax.set_xlabel("Layer")
        ax.text(-0.01, 1.04, f"({'abcdefgh'[idx]})", transform=ax.transAxes,
