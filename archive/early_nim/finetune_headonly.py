"""Head-only fine-tuning: resume from a memorized checkpoint, freeze the
transformer body, and train ONLY the unembedding (lm_head / embed_out) +
final layernorm.

The probe experiment showed the body encodes `final_pile mod (mr+1)` at
~1.0 accuracy in the last layer, but the trained LM head decodes it to the
wrong token. This script tests whether letting just the head learn with
low WD recovers eval accuracy.

CLI:
    python finetune_headonly.py <mr> [<src_wd_str>] [<src_step>]
Defaults: src_wd=1.0, src_step=50000
Examples:
    python finetune_headonly.py 3
    python finetune_headonly.py 6 0.5 50000
"""
...
SRC_WD_STR = sys.argv[2] if len(sys.argv) > 2 else "1.0"
SRC_STEP = int(sys.argv[3]) if len(sys.argv) > 3 else 50000

# Fixed for this experiment
SEED = 42
MODEL_SIZE = "410m"
LR = 1e-4         # head is tiny, can take higher LR
WD = 0.01         # low: preserve / let-grow decoding directions
MAX_STEPS = 20000

SRC_LR_STR = "5e-6"
SRC_REPO = f"ijinyu1113/ft_mr{MAX_REMOVE}_{MODEL_SIZE}_seed{SEED}_lr{SRC_LR_STR}_wd{SRC_WD_STR}_purenum"
...
TAG = f"mr{MAX_REMOVE}_{MODEL_SIZE}_seed{SEED}_headonly_from_wd{SRC_WD_STR}_step{SRC_STEP}"
...
print(f"Head-only FT:  mr={MAX_REMOVE}  src={SRC_REPO}@{SRC_REVISION}")
print(f"  LR={LR}  WD={WD}  MAX_STEPS={MAX_STEPS}")
...
# --- Load model + tokenizer from src checkpoint ---
...
# --- Freeze body; train only head + final layernorm ---
trainable, frozen = 0, 0
for name, param in model.named_parameters():
    if name.startswith("embed_out") or name.startswith("gpt_neox.final_layer_norm"):
